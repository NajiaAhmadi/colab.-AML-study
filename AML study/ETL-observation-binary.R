...
selected_mapping <- subset(mapping, mapping$"Transformation_name" == "Observation_binary")
...
for (i in 1:nrow(vertical_table)) {
  ...
  observation_concept_id <- vertical_table$Concept_id[i]
  observation_date <- "01.01.1800"
  observation_type_concept_id <- "32817"
  observation_source_value <- vertical_table$Variable[i]
